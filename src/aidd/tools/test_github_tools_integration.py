...
from .github_tools import handle_get_issue, handle_create_pull_request_review, handle_get_pull_request_files, handle_list_issues, handle_list_pull_requests
...
@pytest.mark.asyncio
async def test_handle_list_issues_real_api():
    """Test the list_issues tool against the real GitHub API"""
    ...
    assert isinstance(result[0], TextContent)
    assert result[0].type == "text"
    assert "Issues in python/cpython (open)" in result[0].text
    # Check for table headers
    assert "| Number | Title | State | Creator | Labels | Created At |" in result[0].text
    # Check for summary
    assert "Total issues found:" in result[0].text


@pytest.mark.asyncio
async def test_handle_list_issues_closed_real_api():
    """Test the list_issues tool with closed issues against the real GitHub API"""
    result = await handle_list_issues({
        "owner": "python",
        "repo": "cpython",
        "state": "closed",
    ...
    assert isinstance(result[0], TextContent)
    assert result[0].type == "text"
    assert "Issues in python/cpython (closed)" in result[0].text
    # Check for table headers
    assert "| Number | Title | State | Creator | Labels | Created At |" in result[0].text
    # Check for summary
    assert "Total issues found:" in result[0].text
...
@pytest.mark.asyncio
async def test_handle_list_issues_filtered_real_api():
    """Test the list_issues tool with filters against the real GitHub API"""
    result = await handle_list_issues({
        "owner": "python",
        "repo": "cpython",
        "state": "open",
    ...
    # We can't assert that "documentation" is in every issue since the API might change,
    # but we can check if the response is well-formed


@pytest.mark.asyncio
async def test_handle_list_issues_not_found_real_api():
...
@pytest.mark.asyncio
async def test_handle_list_issues_private_repo():
    """Test handling of private repository access"""
    result = await handle_list_issues({
        "owner": "github",
        "repo": "github",  # This is a private repository
